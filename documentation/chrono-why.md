...
After continuous use and development over the months MSSH.chrono finally gets its first official release. This release is neither bug-free nor complete. It does however have all the core functionality that is planned for the near future AND all the data collected in this version will be useable in future releases.
...
Chrono is saving, managing and analyzing your day, deadlines, sleep patterns and workouts. Chrono requires no connection to the internet<sup id="a1">[1](#f1)</sup> and doesn't share any of your data! It works best if it has all kinds of different data, but only one kind of data is needed for Chrono to be useful. The core idea is to split each day into ChronoEvents. You can visualize your days like a calendar or check for patterns in your behavior. You could for example use the heat map and plot features to check how you manage your time around deadlines or to analyze the time frames in which you typically do X. [Images]

Chrono is a basically a command line. There are no buttons, just you and your keyboard. If you feel like you are typing the same command over and over again, but you are only changing one argument, Chrono also features aliases (including currying!) to eliminate such repetitive inputs. 
...
<b id="f1">1</b> If you want to use data imported from your oura ring, you obviously need internet for the duration of the importation.[↩](#a1)
